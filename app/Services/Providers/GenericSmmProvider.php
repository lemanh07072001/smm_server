 ...
 *
 * Handles the standard SMM panel API format used by most providers:
 * - POST to api_url (from DB)
 * - JSON body with key/action/service/link/quantity
 * - Status field "orders" (comma-separated IDs)
 * - Response: {"order": 123} for add, nested/flat for status
 *
 * Providers using this class: trao_doi_tuong_tac, smm, smmking, smmapir, smmzone, ...
 * Each provider differs only by the api_url stored in the database.
 */
class GenericSmmProvider extends BaseProvider
{
    ...
    public function canceledOrder(string|array $orderIds): array
    {
        ...
        try {
            Log::info('GenericSmm Cancel Order Request', [
            ...
            Log::error('GenericSmm Cancel Order Error', [
                'provider' => $this->provider->code,
                'error'    => $e->getMessage(),
            ]);
    ...
    public function parseStatusResponse(array $response): array
    {
        ...
        // Single order (flat response): {"status": "In progress", "charge": ...}
        ...
        // Batch (nested by order ID): {"123": {"status": "...", ...}, ...}
